...
def load_history(path: Path, max_messages: Optional[int] = None) -> list[dict[str, str]]:
    if not path.exists():
        return []
    try:
...
def load_summary_state(path: Path) -> dict[str, object]:
    if not path.exists():
        return {"summary_text": "", "summarized_count": 0}
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError):
        return {"summary_text": "", "summarized_count": 0}
    ...
    summarized_count = raw.get("summarized_count", 0)
    ...
    try:
        ...
        summarized_count = 0
    return {"summary_text": summary_text, "summarized_count": max(0, summarized_count)}


def save_summary_state(path: Path, summary_state: dict[str, object]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    safe_state = {
        "summary_text": str(summary_state.get("summary_text", "")),
        "summarized_count": int(summary_state.get("summarized_count", 0)),
    }
    path.write_text(
